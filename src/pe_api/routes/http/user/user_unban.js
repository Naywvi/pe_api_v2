...
module.exports = {
  ...
  run: async (req, res) => {
    try {
      ...
      if (request_veracity.user_id === request_veracity.sender._id) return res.status(400).json("You can't unfire yourself");
      ...
      //<== read the user(s)
      const result = await check_user_request.unban_one(request_veracity);
